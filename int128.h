...
#error endian not support
	...
	struct clz_helper<unsigned long> {
		...
		static constexpr int clz(unsigned long val_) { unsigned long index = 0; return _BitScanReverse(&index, val_) ? 31 - index : 32; }
#else
...
#ifdef _WIN32
		//get the count of leading zeros
		static constexpr int clz(unsigned long long val_) { unsigned long index = 0; return _BitScanReverse64(&index, val_) ? 63 - index : 64; }
#else
